...
/**
 * Get the archived news articles per year.
...
/**
 * Get the months with arctiled news of a year.
 * @returns {Promise<number[]>}
 */
...
export async function getLatestNews(newsCount=6) {
  return getAllPosts().then( (news) => (news.sort( (a,b) => (new Date(a[1].date) - new Date(b[1].date))).slice(0,newsCOunt-1)));
}
